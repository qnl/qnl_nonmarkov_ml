...
class MultiTimeStep():
    def __init__(self, training_features, training_labels, validation_features, validation_labels, prep_state,
                 n_levels, lstm_neurons=32, mini_batch_size=500, savepath=None, experiment_name='', **kwargs):
    ...
    def prep_state_encoding(self, n_levels, prep_state):
        ...
        # Preparation state information for qutrits (n_levels = 3)
        ...
        if n_levels == 2:
            if prep_state in prep_dict.keys():
                self.prep_x = prep_dict[prep_state]["prep_x"]
                self.prep_y = prep_dict[prep_state]["prep_y"]
                self.prep_z = prep_dict[prep_state]["prep_z"]
